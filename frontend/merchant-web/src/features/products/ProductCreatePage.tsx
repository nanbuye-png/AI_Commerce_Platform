...
const ProductCreatePage: React.FC = () => {
  ...
  const addImage = () => {
    ...
    setImages([...images, { url: imageUrl.trim(), isCover: images.length === 0 }]);
    setImageUrl('');
  };
      ...
      <div style={{ background: 'var(--color-bg-primary)', borderRadius: 'var(--radius-md)', padding: 'var(--spacing-xl)', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ marginBottom: 'var(--spacing-lg)' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="输入图片 URL" style={inputStyle} />
            <button onClick={addImage} style={{ padding: '0 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--color-border)', background: 'transparent', cursor: 'pointer', whiteSpace: 'nowrap' }}>添加</button>
          </div>
          {images.length > 0 && (
            <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
              {images.map((img, i) => (
                <div key={i} style={{ position: 'relative', width: 64, height: 64, borderRadius: 'var(--radius-sm)', overflow: 'hidden', border: img.isCover ? '2px solid var(--color-accent)' : '1px solid var(--color-border)' }}>
                  ...
                  <button onClick={() => removeImage(i)} style={{ position: 'absolute', top: 0, right: 0, width: 18, height: 18, background: 'rgba(0,0,0,0.6)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '11px', lineHeight: '18px' }}>×</button>
                </div>
              ))}
            </div>
          )}
